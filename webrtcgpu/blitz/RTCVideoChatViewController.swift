...
import UIKit
import AVFoundation
class RTCVideoChatViewController: UIViewController, RTCEAGLVideoViewDelegate, ARDAppClientDelegate,RTCMessageReceiver {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    self.switcherOut.setOn(false, animated:true)
    self.switcherOut.addTarget(self, action: Selector("onToggle:"), forControlEvents: UIControlEvents.ValueChanged);
    self.isZoom = false;
  ...
  override func viewWillAppear(animated: Bool) {
    ...
    self.client!.connectToRoomWithId(self.roomName! as String, options: nil, messageReceiver: self)
  ...
  func didReceiveMessage(tag: String!, data: [String : NSObject]!) {
    print("Reciewed message: with tag ", tag, data);
  }
  ...
  func onToggle(switchState: UISwitch){
    if switchState.on {
        print("Mask On");
        client?.sendData("custom_test_tag", data: ["message": "Mask On Message"])
//        guard let channel = client?.channel else {
//            print("Error: chanel doesnt exit yet!");
//            return
//        };
//        channel.sendData("Mask On Message".dataUsingEncoding(NSUTF8StringEncoding));
    } else {
        print("Mask Off");
    }
  }
